    ...
    fTweet -> ask fTweet [
      ...
      ask tweet-sharer [
        ask link-neighbors [  ; linked to the sharer
          if (user-positioning >= tweet-positioning-retweet) and (user-mood <= tweet-emotion-retweet) [  ; !!!!!!! need math - makes no sense the comparation
             set retweet? true
...
to refresh-links-follow
  ask users [
    ...
    print linked-friends

    let listed-users []
    foreach retweet-list [
      retweet -> ask retweet [
          set listed-users lput tweet-owner listed-users
      ]
    ]

    foreach listed-users [
      listed-user -> ask listed-user [
        if not member? listed-user linked-friends [
          ;create-link-with listed-user ; NAO É DIRECIONADO
          print "------- NEW LINK -------"
          print listed-user
          print "------- ME -------"
          print user-analysed
